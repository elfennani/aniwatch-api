...
const showByID: FastifyPluginAsync = async (fastify, opts): Promise<void> => {
  fastify.get("/:id", async function (request, reply): Promise<ShowDetails> {
    ...
    const show = showSearch!!.shows.edges?.find((show) => show.aniListId == id);
    ...
    if (show && showId) {
      ...
      episodes = show.availableEpisodesDetail.sub.map((ep) => {
        const details = allAnimeEpisodes.episodeInfos.find(
          (info) => info.episodeIdNum.toString() === ep
        );
        const anilistDetails = media?.streamingEpisodes?.find((episode) => {
          return episode?.title?.startsWith(`Episode ${ep} `);
        });
        ...
        return {
          ...
          name: anilistDetails?.title ?? `Episode ${ep}`,
          thumbnail:
            anilistDetails?.thumbnail ??
            details?.thumbnails
              ?.filter((t) => !t.includes("cdnfile"))
              ?.map((t) => (t.startsWith("http") ? t : source + t))?.[0],
          duration: details?.vidInforssub?.vidDuration,
          dubbed:
            show.availableEpisodesDetail.dub.includes(String(ep)) ?? false,
        };
      });
    }
